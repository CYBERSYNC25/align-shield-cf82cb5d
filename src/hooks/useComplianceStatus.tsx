...
const COMPLIANCE_RULES: ComplianceRule[] = [
  ...
  // Manual Entry Rules
  {
    id: 'manual-user-no-mfa',
    title: 'Usuário Manual sem MFA',
    description: 'Usuário adicionado manualmente sem autenticação multifator',
    severity: 'high',
    integrationId: 'manual-entry',
    resourceType: 'user',
    checkFn: (resource) => {
      const data = resource.resource_data;
      return data?.mfaEnabled === false && data?.accountStatus === 'active';
    },
    getAffectedName: (resource) => resource.resource_data?.name || resource.resource_data?.email || 'User',
    fixAction: '/integrations',
  },
  {
    id: 'manual-repo-public',
    title: 'Repositório Manual Público',
    description: 'Repositório adicionado manualmente com visibilidade pública',
    severity: 'critical',
    integrationId: 'manual-entry',
    resourceType: 'repository',
    checkFn: (resource) => resource.resource_data?.visibility === 'public',
    getAffectedName: (resource) => resource.resource_data?.name || 'Repository',
    fixAction: '/integrations',
  },
  {
    id: 'manual-repo-no-protection',
    title: 'Repositório Manual sem Proteção de Branch',
    description: 'Repositório adicionado manualmente sem proteção de branch habilitada',
    severity: 'high',
    integrationId: 'manual-entry',
    resourceType: 'repository',
    checkFn: (resource) => resource.resource_data?.branchProtection === false,
    getAffectedName: (resource) => resource.resource_data?.name || 'Repository',
    fixAction: '/integrations',
  },
  {
    id: 'manual-bucket-public',
    title: 'Storage Manual Público',
    description: 'Bucket/Storage adicionado manualmente com acesso público',
    severity: 'critical',
    integrationId: 'manual-entry',
    resourceType: 'bucket',
    checkFn: (resource) => resource.resource_data?.publicAccess === true,
    getAffectedName: (resource) => resource.resource_data?.name || 'Storage',
    fixAction: '/integrations',
  },
  {
    id: 'manual-bucket-no-encryption',
    title: 'Storage Manual sem Criptografia',
    description: 'Bucket/Storage adicionado manualmente sem criptografia habilitada',
    severity: 'high',
    integrationId: 'manual-entry',
    resourceType: 'bucket',
    checkFn: (resource) => resource.resource_data?.encryption === false,
    getAffectedName: (resource) => resource.resource_data?.name || 'Storage',
    fixAction: '/integrations',
  },
  {
    id: 'manual-db-public',
    title: 'Database Manual Público',
    description: 'Banco de dados adicionado manualmente com acesso público',
    severity: 'critical',
    integrationId: 'manual-entry',
    resourceType: 'database',
    checkFn: (resource) => resource.resource_data?.publicAccess === true,
    getAffectedName: (resource) => resource.resource_data?.name || 'Database',
    fixAction: '/integrations',
  },
  {
    id: 'manual-db-no-encryption',
    title: 'Database Manual sem Criptografia',
    description: 'Banco de dados adicionado manualmente sem criptografia em repouso',
    severity: 'high',
    integrationId: 'manual-entry',
    resourceType: 'database',
    checkFn: (resource) => resource.resource_data?.encryptionAtRest === false,
    getAffectedName: (resource) => resource.resource_data?.name || 'Database',
    fixAction: '/integrations',
  },
  {
    id: 'manual-server-no-encryption',
    title: 'Servidor Manual sem Criptografia',
    description: 'Servidor/VM adicionado manualmente sem criptografia de disco',
    severity: 'high',
    integrationId: 'manual-entry',
    resourceType: 'server',
    checkFn: (resource) => resource.resource_data?.encryptionEnabled === false,
    getAffectedName: (resource) => resource.resource_data?.hostname || 'Server',
    fixAction: '/integrations',
  },
  {
    id: 'manual-app-no-https',
    title: 'Aplicação Manual sem HTTPS',
    description: 'Aplicação web adicionada manualmente sem HTTPS habilitado',
    severity: 'critical',
    integrationId: 'manual-entry',
    resourceType: 'application',
    checkFn: (resource) => {
      const data = resource.resource_data;
      return data?.type === 'Web' && data?.httpsEnabled === false;
    },
    getAffectedName: (resource) => resource.resource_data?.name || 'Application',
    fixAction: '/integrations',
  },
];
