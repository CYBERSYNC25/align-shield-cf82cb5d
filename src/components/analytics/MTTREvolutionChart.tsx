...
export function MTTREvolutionChart() {
  ...
  const chartData = useMemo(() => {
    if (!alerts || alerts.length === 0) {
      ...
      const weeks = Array.from({ length: 8 }, (_, i) => {
        ...
        return {
          ...
          critical: 4 + Math.random() * 8,
          high: 12 + Math.random() * 24,
          medium: 48 + Math.random() * 72,
        };
      });
      return weeks;
    }
    ...
    const groupedByWeek = alerts.reduce((acc, alert) => {
      ...
      if (!acc[weekKey]) {
        acc[weekKey] = { critical: [], high: [], medium: [] };
      }
    ...
    return Object.entries(groupedByWeek).map(([week, values]) => ({
      ...
      critical: values.critical.length > 0 
        ? Math.round(values.critical.reduce((a, b) => a + b, 0) / values.critical.length) 
        : null,
      high: values.high.length > 0 
        ? Math.round(values.high.reduce((a, b) => a + b, 0) / values.high.length) 
        : null,
      medium: values.medium.length > 0 
        ? Math.round(values.medium.reduce((a, b) => a + b, 0) / values.medium.length) 
        : null,
    }));
  }, [alerts]);
      ...
      <CardContent>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <ReferenceLine 
                ...
                strokeDasharray="5 5" 
                label={{ value: 'SLA Crítico (24h)', fill: 'hsl(var(--muted-foreground))', fontSize: 10 }} 
              />
              <Line
                type="monotone"
                dataKey="critical"
        ...
        <div className="flex flex-wrap gap-4 mt-4 text-xs text-muted-foreground">
          <div className="flex items-center gap-1">
            ...
            <span>Crítico: SLA 24h</span>
          </div>
          <div className="flex items-center gap-1">
            <AlertCircle className="h-3 w-3 text-orange-500" />
            <span>Alto: SLA 7 dias</span>
          </div>
          <div className="flex items-center gap-1">
            <AlertCircle className="h-3 w-3 text-primary" />
            <span>Médio: SLA 30 dias</span>
          </div>
        </div>
      </CardContent>
    </Card>
