        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {auditorSessions.length === 0 ? (
            ...
            auditorSessions.map((session) => (
              <Card key={session.auditId} className="bg-background border-border">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center space-x-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <CardTitle className="text-sm truncate">{session.auditor}</CardTitle>
                ...
                <CardContent className="space-y-3 pt-0">
                  ...
                  {/* Activity Stats */}
                  <div className="grid grid-cols-2 gap-3 p-2 bg-muted/20 rounded-lg">
                    <div className="text-center">
                      <div className="text-base font-bold text-foreground">{session.progress}%</div>
                      <div className="text-xs text-muted-foreground">Progresso</div>
                    </div>
                    <div className="text-center">
                      <div className="text-base font-bold text-foreground">{session.evidenceAccessed}</div>
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        toast({
                          ...
                          description: "Redirecionando para o portal externo...",
                        });
                        setTimeout(() => {
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        toast({
                          ...
                          description: "Baixando log de atividades...",
                        });
                        setTimeout(() => {
                          const blob = new Blob([`Log de atividades - ${session.auditor}\n\nData: ${new Date().toLocaleString()}\nAções: ${Math.floor(Math.random() * 50) + 10}`], { type: 'text/plain' });
                          const url = window.URL.createObjectURL(blob);
                          const a = document.createElement('a');
                          a.href = url;
                          a.download = `log-${session.auditor.replace(' ', '-').toLowerCase()}.txt`;
